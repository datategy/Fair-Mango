...
from fair_mango.metrics.superset import SupersetFairnessMetrics, SupersetPerformanceMetrics
import json
...
def create_interface_test_scenarios() -> List[Dict[str, Any]]:
    ...
    datasets_info = {
        ...
        'hiring': {
            ...
            'targets': {
                'Hired': {'pred': 'HiredPred', 'positive': 1}
            }
        },
        'loan_approval': {
            ...
            'targets': {
                'Approved': {'pred': 'ApprovedPred', 'positive': 1}
            }
        },
        'criminal_justice': {
            ...
            'targets': {
                'Recidivism': {'pred': 'RecidivismPred', 'positive': 1}
            }
        },
        'college_admission': {
            ...
            'sensitive_attrs': ['Race', 'Gender'],
            'targets': {
                'Admitted': {'pred': 'AdmittedPred', 'positive': 1}
            }
        }
    }
        ...
        try:
            ...
            for target_name, target_info in info['targets'].items():
                ...
                if len(info['sensitive_attrs']) >= 2:
                    print(f"Creating multiple_sensitive scenario for {dataset_name} with attributes: {info['sensitive_attrs'][:2]}")
                    scenario = create_single_scenario(
                        dataset_name=dataset_name,
                        df=df,
...
def create_single_scenario(dataset_name: str, df: pd.DataFrame, sensitive: List[str], 
                          target: str, predicted: str = None, positive = None,
    ...
    try:
        ...
        for group in groups_data:
            ...
            scenario['input']['sensitive_groups'][group_key] = {
                'sensitive_group_values': group.sensitive_group,
                'size': len(group.data),
        ...
        if predicted is not None:
            try:
                ...
                for result in fairness_results:  # Include all combinations
                    ...
                    # Clean sensitive_group representation
                    if isinstance(result_dict.get('sensitive_group'), list):
                        if len(result_dict['sensitive_group']) == 1:
                            result_dict['sensitive_group_key'] = result_dict['sensitive_group'][0]
                        else:
                            result_dict['sensitive_group_key'] = "_".join(result_dict['sensitive_group'])
                        result_dict['sensitive_group_values'] = result_dict['sensitive_group']
                    ...
                    if 'rankings' in result_dict:
                        ...
                        for metric_name, rankings in result_dict['rankings'].items():
                            ...
                            for rank in rankings:
                                clean_rank = dict(rank)
                                if isinstance(clean_rank.get('sensitive_group'), list):
                                    if len(clean_rank['sensitive_group']) == 1:
                                        clean_rank['sensitive_group_key'] = clean_rank['sensitive_group'][0]
                                    else:
                                        clean_rank['sensitive_group_key'] = "_".join(clean_rank['sensitive_group'])
                                    clean_rank['sensitive_group_values'] = clean_rank['sensitive_group']
                ...
                scenario['outputs']['fairness_metrics'] = {
                    'available_metrics': list(fairness_results[0].rankings.keys()) if fairness_results else [],
            ...
            try:
                ...
                for result in performance_results:  # Include all combinations
                    ...
                    if isinstance(result.sensitive_group, (list, tuple)):
                        if len(result.sensitive_group) == 1:
                            clean_result['sensitive_group_key'] = str(result.sensitive_group[0])
                        else:
                            clean_result['sensitive_group_key'] = "_".join(str(x) for x in result.sensitive_group)
                        clean_result['sensitive_group_values'] = list(result.sensitive_group)
                    else:
                        clean_result['sensitive_group_key'] = str(result.sensitive_group)
                        clean_result['sensitive_group_values'] = [str(result.sensitive_group)]
                    
                    clean_performance_results.append(clean_result)
                
                scenario['outputs']['performance_metrics'] = {
                    'available_metrics': ['selection_rate', 'performance_metric', 'confusion_matrix'],
        ...
        if len(sensitive) == 1:
            ...
            scenario['outputs']['basic_statistics']['sensitive_group_counts'] = group_counts
        else:
            ...
            scenario['outputs']['basic_statistics']['sensitive_group_counts'] = clean_group_counts
...
def save_interface_documentation(scenarios: List[Dict], examples: Dict, metrics: Dict, 
                               output_dir: str = "tests/data") -> None:
    """
...
def create_interface_guide(scenarios: List[Dict], examples: Dict, metrics: Dict, 
