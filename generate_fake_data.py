...
def create_heart_disease_dataset(n_samples: int = 918) -> pd.DataFrame:
    ...
    sex = np.random.choice(['M', 'F'], n_samples, p=[0.79, 0.21])  # Male-heavy as in real data
    ...
    resting_ecg = np.random.choice(['Normal', 'ST', 'LVH'], n_samples, p=[0.6, 0.2, 0.2])
    ...
    error_indices = np.random.choice(n_samples, int(n_samples * (1 - prediction_accuracy)), replace=False)
...
def create_loan_approval_dataset(n_samples: int = 1500) -> pd.DataFrame:
    ...
    race = np.random.choice(['White', 'Black', 'Hispanic', 'Asian'], 
                           n_samples, p=[0.65, 0.15, 0.12, 0.08])
    ...
    debt_to_income = np.random.beta(2, 5, n_samples) * 0.8  # Most people have reasonable DTI
...
def create_criminal_justice_dataset(n_samples: int = 800) -> pd.DataFrame:
    ...
    crime_type = np.random.choice(['Violent', 'Property', 'Drug', 'Other'], 
    ...
    employment_status = np.random.choice(['Unemployed', 'Part-time', 'Full-time'], 
...
def create_college_admission_dataset(n_samples: int = 1200) -> pd.DataFrame:
    ...
    """
    # Demographics
    race = np.random.choice(['White', 'Asian', 'Hispanic', 'Black', 'Other'], 
                           n_samples, p=[0.45, 0.25, 0.15, 0.12, 0.03])
...
def save_datasets(datasets: Dict[str, pd.DataFrame], output_dir: str = "tests/data") -> None:
    """
    ...
    for name, df in datasets.items():
        ...
        print(f"Saved {name} dataset: {df.shape[0]} rows, {df.shape[1]} columns -> {file_path}")
